...
Задание: разработать полноценный веб-сервер

-	Подключить базу данных
-	Реализовать полный набор CRUD операций над продуктами (все действия должны влиять на продукты в базе данных):
```
[x]  GET /items – получение всех продуктов
[x]  GET /items?id=1 – получение продукта, с указанным id (***** или более правильный, но сложный вариант /items/{id} – использование PathVariable)
[x]  POST /items – создание нового продукта
[x]  PUT /items – модификация продукта
[x]  DELETE /items?id=1 – удаление продукта с указанным id
```
-	При запросе GET /[имя_файла] возвращать в теле ответа указанный файл из папки static (надо оптимизировать) *
...
-	Если есть обработчик GET /something, но нет обработчика запроса POST /something, то при POST запросе необходимо кинуть ответ 405 Method Not Allowed (это значит что вы можете впринципе обрабатывать указанный uri, только с другим http-методом)
